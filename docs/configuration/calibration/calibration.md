...
The `calibrate_model.sh` script automatically generates appropriate configuration files for the calibration process. However, if you require advanced customization, you can use JSON configuration files to control aspects such as scale calculation methods, module selection, and performance optimization. The `quantization_config` directory contains JSON templates that you can use directly or modify to suit your specific requirements.

To apply custom configurations for calibration, manually run the calibration steps (`step-2-measure-scales.py` and `step-4-quantize-scales.py`) with the `QUANT_CONFIG` environment variable pointing to your configuration file. To apply the configuration when running FP8 inference, set `QUANT_CONFIG` to point to the quantization configuration file, either the one generated by calibration or your custom configuration.
